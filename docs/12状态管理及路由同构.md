...
```markdown
# example 参见仓库folder 12
目录结构
- dist
  - build.js // entry-client.js 打包结果
- src
  - About.vue
  - App.vue
  - entry-client.js // csr的js文件
  - entry-server.js // ssr的node服务端js
  - Home.vue
  - index.js // csr和ssr的创建vue实例的共用js
  - index.template.html
- .babel.rc // babel的runtime config
- package.json // 建议yarn安装依赖
- server.bundle.js // entry-server.js 打包结果
- webpack.config.js
